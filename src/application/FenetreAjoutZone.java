...
public class FenetreAjoutZone extends Stage {
	...
	/**
	 * D̩finit les param̬tre de la fenetre AjoutZone
	 */
	public FenetreAjoutZone() {
		this.setTitle("Ajouter une zone");
	...
	private Parent creerContenu() {
		...
		//ferme la fenetre si on appuie sur le bouton annuler
		bnAnnuler.setOnAction(e -> {
			this.close();
		});
		
		//ajoute la zone dans la liste puis ferme la fenetre si on appuie sur le bouton OK
		bnOK.setOnAction(e -> {
			Zone zone = new Zone(txtNom.getText());
			Principale.ajouterZone(zone);
			this.close();
		});
	 ...
	 * @return un bool̩en qui est vrai si le nom est contenu dans la liste des zones laListe
	 */
	public boolean listeZoneContientNomZone(String nom, ObservableList<Zone> laListe) {
		boolean contient = false;
		int i=0;
		while (!contient && i<laListe.size()) {
			if (laListe.get(i).getNomZone().equals(nom)) {
				contient=true;
			}
			else {
				i++;
